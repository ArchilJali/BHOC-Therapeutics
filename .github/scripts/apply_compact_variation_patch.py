...
def replace(path, old, new, count=1):
    ...
    if old not in s: raise SystemExit(f'Expected text not found in {path}: {old[:100]}')
...
# Refresh only overview text fingerprint; all source paragraph/link protections remain.
...
html=(ROOT/'bhoc/index.html').read_text();p=MainText();p.feed(html);norm=' '.join(' '.join(x.strip() for x in p.parts if x.strip()).split())
bp=ROOT/'bhoc/knowledge-map-baseline.json';b=json.loads(bp.read_text());b['pages']['bhoc/index.html']['textSha256']=hashlib.sha256(norm.encode()).hexdigest();bp.write_text(json.dumps(b,indent=2)+'\n')
print('Applied compact cards and Human/Animal Variation without touching head metadata, sitemap or chapter prose.')
